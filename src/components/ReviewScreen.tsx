...
export const ReviewScreen: React.FC<ReviewScreenProps> = ({ reportData, onEditSection, onFinalize }) => {
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-12">
      ...
      <div id="review-report-printable" className="bg-white border border-slate-200 rounded-2xl p-5 space-y-6 shadow-sm">
        
        {/* Header Block */}
        <div className="border-b border-slate-200 pb-4 flex flex-wrap justify-between items-start gap-3">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">{r.shutdownName || 'Shutdown Event'}</p>
          </div>

          <div className="text-right text-xs space-y-1">
            <div><span className="text-slate-400">Date:</span> <strong className="text-slate-800">{r.date || 'N/A'}</strong></div>
            <div><span className="text-slate-400">Technician:</span> <strong className="text-slate-800">{r.technicianName || 'N/A'} ({r.technicianId || 'N/A'})</strong></div>
            <div><span className="text-slate-400">Type:</span> <strong className="text-amber-600">{r.failureType || 'Mechanical'}</strong></div>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between bg-slate-50 p-2.5 rounded-lg border border-slate-200">
            <h3 className="font-bold text-xs text-slate-800">1. Basic Equipment & Location Info</h3>
            <button
              onClick={() => onEditSection(0)}
              className="text-sky-600 font-bold text-xs hover:underline flex items-center space-x-1"
            >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs bg-slate-50/50 p-3 rounded-lg border border-slate-100">
            <div><span className="text-slate-400 block text-[10px]">Equipment Code</span><strong className="text-slate-800 font-mono">{r.equipmentCode || 'N/A'}</strong></div>
            <div><span className="text-slate-400 block text-[10px]">Plant Area</span><strong className="text-slate-800">{r.location || 'N/A'}</strong></div>
            <div><span className="text-slate-400 block text-[10px]">Failure Classification</span><strong className="text-slate-800">{r.failureType || 'N/A'}</strong></div>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-1.5 text-xs">
            <div className="p-2 bg-slate-50 rounded border border-slate-200"><span className="font-bold text-purple-700 mr-2">Why #1:</span>{fiveWhy.why1 || 'N/A'}</div>
            <div className="p-2 bg-slate-50 rounded border border-slate-200"><span className="font-bold text-purple-700 mr-2">Why #2:</span>{fiveWhy.why2 || 'N/A'}</div>
            <div className="p-2 bg-slate-50 rounded border border-slate-200"><span className="font-bold text-purple-700 mr-2">Why #3:</span>{fiveWhy.why3 || 'N/A'}</div>
            <div className="p-2 bg-slate-50 rounded border border-slate-200"><span className="font-bold text-purple-700 mr-2">Why #4:</span>{fiveWhy.why4 || 'N/A'}</div>
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
            ...
            <div className="border border-slate-200 rounded-xl p-3 bg-slate-50/50">
              <span className="font-bold text-slate-800 block mb-2">Corrective Action Items</span>
              {actions.length === 0 ? (
                <p className="text-slate-400 italic">No actions logged</p>
              ) : (
                <ul className="space-y-1.5">
                  {actions.map((a, idx) => (
                    <li key={a.id || idx} className="p-2 bg-white rounded border border-slate-200 flex justify-between items-center text-[11px]">
                      <div>
                        <p className="font-medium text-slate-800">{a.action}</p>
                        <span className="text-[10px] text-slate-400">Assigned: {a.assignee}</span>
                      </div>
            ...
            <div className="border border-slate-200 rounded-xl p-3 bg-slate-50/50">
              <div className="flex justify-between items-center mb-2">
                <span className="font-bold text-slate-800">Spare Parts Inventory</span>
                <span className="font-mono font-bold text-emerald-600">${totalPartCost.toFixed(2)}</span>
              </div>
              {parts.length === 0 ? (
                <p className="text-slate-400 italic">No parts logged</p>
              ) : (
                <ul className="space-y-1.5">
                  {parts.map((p, idx) => (
                    <li key={p.id || idx} className="p-2 bg-white rounded border border-slate-200 flex justify-between items-center text-[11px]">
                      <div>
                        <p className="font-medium text-slate-800">{p.partName}</p>
                        <span className="text-[10px] font-mono text-slate-400">{p.partNumber} ({p.quantity}x)</span>
                      </div>
                      <span className="font-mono font-bold text-slate-800">${(p.quantity * p.unitCost).toFixed(2)}</span>
                    </li>
                  ))}
                </ul>
              )}
        ...
        {photos.length > 0 && (
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {photos.map((ph, idx) => (
                <div key={ph.id || idx} className="border border-slate-200 rounded-xl overflow-hidden bg-slate-900 shadow-sm">
                  ...
                  <div className="p-2.5 bg-white text-xs text-slate-700 border-t border-slate-200">
                    <p className="font-bold text-slate-800">{ph.caption || `Inspection Photo #${idx + 1}`}</p>
                    <span className="text-[10px] text-slate-400 font-mono block mt-0.5">{ph.timestamp || 'Recorded'}</span>
                  </div>
                </div>
              ))}
            </div>
