  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="space-y-3">
        ...
        {actions.length === 0 ? (
          <div className="p-4 bg-[var(--panel)] border border-dashed border-[var(--panel-border)] rounded-xl text-center text-xs text-[var(--text-2)]">
            No corrective actions added yet. Click &quot;Add Action&quot; to log repair steps.
          </div>
        ) : (
          <div className="space-y-2.5">
            {actions.map((ca, idx) => (
              <div key={ca.id} className="p-3 bg-[var(--panel-solid)] border border-[var(--panel-border)] rounded-xl space-y-2 shadow-xs">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold text-[var(--text-2)] uppercase">Action #{idx + 1}</span>
                  <button
                    onClick={() => removeAction(ca.id)}
                    className="p-1 text-[var(--text-2)] hover:text-rose-600 rounded transition-colors"
                    title="Remove action"
                  >
                ...
                <input
                  ...
                  className="w-full bg-[var(--bg-1)] border border-[var(--panel-border)] rounded-lg p-2 text-xs text-[var(--text-0)] focus:outline-none focus:bg-[var(--panel-solid)] focus:ring-2 focus:ring-[var(--accent)]"
                />

                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                  <div>
                    <label className="text-[10px] text-[var(--text-2)] block mb-0.5">Assignee</label>
                    <input
                      type="text"
                      value={ca.assignee}
                      onChange={(e) => updateAction(ca.id, { assignee: e.target.value })}
                      className="w-full bg-[var(--bg-1)] border border-[var(--panel-border)] rounded p-1.5 text-xs text-[var(--text-0)]"
                    />
                  </div>

                  <div>
                    <label className="text-[10px] text-[var(--text-2)] block mb-0.5">Priority</label>
                    <select
                      value={ca.priority}
                      onChange={(e) => updateAction(ca.id, { priority: e.target.value as any })}
                      className="w-full bg-[var(--bg-1)] border border-[var(--panel-border)] rounded p-1.5 text-xs text-[var(--text-0)] font-medium"
                    >
                  ...
                  <div>
                    <label className="text-[10px] text-[var(--text-2)] block mb-0.5">Status</label>
                    <select
                      value={ca.status}
                  ...
                  <div>
                    <label className="text-[10px] text-[var(--text-2)] block mb-0.5">Target Date</label>
                    <input
                      type="date"
                      value={ca.targetDate}
        ...
        {spareParts.length === 0 ? (
          ...
          <div className="space-y-2.5">
            {spareParts.map((sp, idx) => (
              <div key={sp.id} className="p-3 bg-[var(--panel-solid)] border border-[var(--panel-border)] rounded-xl space-y-2 shadow-xs">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold text-[var(--text-2)] uppercase">Part #{idx + 1}</span>
                  <button
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  <input
                    ...
                    className="bg-[var(--bg-1)] border border-[var(--panel-border)] rounded-lg p-2 text-xs text-[var(--text-0)]"
                  />
                  <input
                    type="text"
                    value={sp.partNumber}
                    ...
                    className="bg-[var(--bg-1)] border border-[var(--panel-border)] rounded-lg p-2 text-xs text-[var(--text-0)] font-mono"
                  />
                </div>

                <div className="grid grid-cols-3 gap-2 text-xs">
                  <div>
                    <label className="text-[10px] text-[var(--text-2)] block mb-0.5">Qty</label>
                    <input
                      type="number"
                      min="1"
                      value={sp.quantity}
                  ...
                  <div>
                    <label className="text-[10px] text-[var(--text-2)] block mb-0.5">Unit Cost ($)</label>
                    <input
                      type="number"
                      min="0"
                      value={sp.unitCost}
            ...
            {/* Total Cost Banner */}
            <div className="p-3 bg-[var(--panel-solid)] text-[var(--text-0)] rounded-xl flex items-center justify-between text-xs font-bold shadow-sm">
