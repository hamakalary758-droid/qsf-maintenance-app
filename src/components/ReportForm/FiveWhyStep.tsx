...
export const FiveWhyStep: React.FC<FiveWhyStepProps> = ({ reportData, onChange, geminiApiKey = '' }) => {
  ...
  const isNA = Boolean(fiveWhy.notApplicable);
  ...
  const isGarbage = (val?: string) => {
    ...
    const placeholders = ['-', 'n/a', 'na', 'none', '.', 'nil', 'null', 'n.a.', 'na/'];
    return placeholders.includes(trimmed) || val.trim().length < 5;
  };
...
WHY5: <Systemic, design, quality, or management root cause>`;
  ...
  const toggleSkipWhy = (skipKey: 'why1Skipped' | 'why2Skipped' | 'why3Skipped' | 'why4Skipped', valKey: 'why1' | 'why2' | 'why3' | 'why4') => {
    const isCurrentlySkipped = Boolean(fiveWhy[skipKey]);
    onChange({
      fiveWhy: {
        ...fiveWhy,
        [skipKey]: !isCurrentlySkipped,
        ...( !isCurrentlySkipped ? { [valKey]: '' } : {} )
      }
    });
  };

  const whySteps: Array<{
    key: 'why1' | 'why2' | 'why3' | 'why4' | 'why5';
    skipKey?: 'why1Skipped' | 'why2Skipped' | 'why3Skipped' | 'why4Skipped';
    label: string;
    prompt: string;
  }> = [
    { key: 'why1', skipKey: 'why1Skipped', label: 'WHY #1', prompt: 'First direct physical observation' },
    { key: 'why2', skipKey: 'why2Skipped', label: 'WHY #2', prompt: 'Immediate sub-assembly or mechanical cause' },
    { key: 'why3', skipKey: 'why3Skipped', label: 'WHY #3', prompt: 'Component level wear or operational stress' },
    { key: 'why4', skipKey: 'why4Skipped', label: 'WHY #4', prompt: 'Operating condition or maintenance practice' },
    { key: 'why5', label: 'WHY #5 (ROOT CAUSE)', prompt: 'Systemic / process / root cause factor' }
  ];
    ...
    <div className="space-y-4 animate-fadeIn">
      ...
      <div className="bg-purple-50 dark:bg-purple-950/30 border border-purple-200 dark:border-purple-800 rounded-xl p-3.5 text-xs text-purple-900 dark:text-purple-200 flex flex-wrap sm:flex-nowrap items-start justify-between gap-3">
        ...
        <button
          ...
          disabled={isGenerating || isNA}
          className="flex-shrink-0 px-3 py-1.5 bg-purple-600 hover:bg-purple-700 text-white font-bold text-xs rounded-lg transition-colors flex items-center space-x-1.5 shadow-sm disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
          title={isNA ? '5-Why is marked as Not Applicable' : 'Auto-suggest 5-Why breakdown based on current report data'}
        >
          {isGenerating ? (
            <>
      ...
      <div className="bg-slate-50 dark:bg-slate-850 border border-slate-200 dark:border-slate-700/80 rounded-xl p-3 flex items-center justify-between gap-3">
        ...
        {isNA && (
          <span className="px-2 py-0.5 bg-purple-100 dark:bg-purple-950/60 text-purple-700 dark:text-purple-300 font-bold text-[10px] uppercase rounded-md border border-purple-200 dark:border-purple-800">
            N/A Active
          </span>
        )}
      </div>
        ...
        <>
          ...
          <div className="space-y-3">
            {whySteps.map((step, idx) => {
              ...
              const isSkipped = step.skipKey ? Boolean(fiveWhy[step.skipKey]) : false;
              const hasGarbage = !isSkipped && isGarbage(fiveWhy[step.key]);
              ...
              return (
                <React.Fragment key={step.key}>
                  ...
                  >
                    <div className="flex items-center justify-between mb-1.5 gap-2">
                      ...
                      >
                        ...
                        {isRootCause && (
                          <>
                            ...
                            <AlertTriangle className="w-3.5 h-3.5 text-rose-600 dark:text-rose-400 ml-1" />
                          </>
                        )}
                      </span>
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        {step.skipKey && (
                          ...
                          >
                            <span>{isSkipped ? 'Skipped (Enable)' : 'Not needed'}</span>
                          </button>
                        )}
                      </div>
                    </div>
                    ...
                    {hasGarbage && (
                      <p className="text-[10px] text-rose-600 dark:text-rose-400 mt-1 flex items-center space-x-1">
                        <AlertTriangle className="w-3 h-3 shrink-0 inline mr-0.5" />
                        <span>Must be a meaningful explanation (minimum 5 characters, cannot be a placeholder like &quot;-&quot; or &quot;n/a&quot;).</span>
                      </p>
                    )}
                  </div>
