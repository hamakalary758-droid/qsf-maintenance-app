...
export const ConflictResolver: React.FC<ConflictResolverProps> = ({
  ...
  return (
    ...
    >
      <div
        className="bg-[var(--panel-solid)] border border-[var(--panel-border)] rounded-3xl max-w-4xl w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden my-auto"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Modal Header */}
        <div className="p-5 sm:p-6 border-b border-[var(--panel-border)] flex items-center justify-between bg-amber-500/10 dark:bg-amber-950/30">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 rounded-2xl bg-amber-500/20 text-amber-600 dark:text-amber-400 flex items-center justify-center shrink-0">
              <AlertTriangle className="w-5 h-5" />
            </div>
            <div>
              <h2 className="text-lg font-bold text-[var(--text-0)] flex items-center gap-2">
          ...
          <button
            onClick={onClose}
            className="p-2 text-[var(--text-2)] hover:text-[var(--text-1)] hover:bg-[var(--bg-2)] rounded-xl transition-colors cursor-pointer"
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
          {isLoading ? (
            <div className="py-12 flex flex-col items-center justify-center space-y-3 text-[var(--text-2)]">
            ...
            <div className="py-12 flex flex-col items-center justify-center text-center space-y-3">
              ...
              <p className="text-xs text-[var(--text-2)] max-w-md">
                All of your reports are in sync with the cloud database. There are no outstanding version mismatches.
              </p>
              <button
                onClick={onClose}
                className="mt-2 px-5 py-2 bg-[var(--bg-2)] hover:bg-[var(--panel-border-strong)] text-[var(--text-0)] text-xs font-bold rounded-xl transition-all shadow-sm cursor-pointer"
              >
                Back to Reports
              </button>
            </div>
                  ...
                  >
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-amber-200/60 dark:border-amber-800/40">
                      <div>
                        ...
                        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1 text-[11px] text-[var(--text-2)]">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="bg-[var(--panel-solid)] border-2 border-sky-300 dark:border-sky-800 rounded-2xl p-4 space-y-3 flex flex-col justify-between shadow-sm">
                        <div className="space-y-3">
                          ...
                          <div className="space-y-1.5 text-xs">
                            <div className="text-[var(--text-2)] text-[11px]">
                              Last edited locally: <strong className="text-[var(--text-0)]">{localUpdated}</strong>
                            </div>
                            <div>
                              <span className="text-[var(--text-2)] text-[11px]">Status:</span>{' '}
                              <span className="font-semibold text-[var(--text-0)]">{local.status || 'Draft'}</span>
                            </div>
                            <div>
                              <span className="text-[var(--text-2)] text-[11px]">Failure Type:</span>{' '}
                              <span className="font-semibold text-[var(--text-0)]">{local.failureType}</span>
                            </div>
                            <div>
                              <span className="text-[var(--text-2)] text-[11px]">Technician:</span>{' '}
                              <span className="font-semibold text-[var(--text-0)]">{local.technicianName || 'N/A'}</span>
                            </div>
                            {local.notes && (
                              <div className="bg-[var(--bg-1)] p-2 rounded-xl text-[11px] text-[var(--text-1)] italic">
                                "{local.notes}"
                              </div>
                            )}
                          </div>
                        ...
                        <div className="pt-3 border-t border-[var(--panel-border)] flex items-center space-x-2">
                          <button
                            type="button"
                      ...
                      <div className="bg-[var(--panel-solid)] border-2 border-emerald-300 dark:border-emerald-800 rounded-2xl p-4 space-y-3 flex flex-col justify-between shadow-sm">
                        <div className="space-y-3">
                          <div className="flex items-center justify-between pb-2 border-b border-[var(--panel-border)]">
                          ...
                          <div className="space-y-1.5 text-xs">
                            <div className="text-[var(--text-2)] text-[11px]">
                              Saved to cloud: <strong className="text-[var(--text-0)]">{serverUpdated}</strong>
                            </div>
                            <div>
                              <span className="text-[var(--text-2)] text-[11px]">Status:</span>{' '}
                              <span className="font-semibold text-[var(--text-0)]">{server.status || 'Draft'}</span>
                            </div>
                            <div>
                              <span className="text-[var(--text-2)] text-[11px]">Failure Type:</span>{' '}
                              <span className="font-semibold text-[var(--text-0)]">{server.failureType}</span>
                            </div>
                            <div>
                              <span className="text-[var(--text-2)] text-[11px]">Technician:</span>{' '}
                              <span className="font-semibold text-[var(--text-0)]">{server.technicianName || 'N/A'}</span>
                            </div>
                            {server.notes && (
                              <div className="bg-[var(--bg-1)] p-2 rounded-xl text-[11px] text-[var(--text-1)] italic">
                                "{server.notes}"
                    ...
                    {isExpanded && (
                      <div className="mt-4 pt-4 border-t border-amber-200/60 dark:border-amber-800/40 space-y-3">
                        <h5 className="text-xs font-bold text-[var(--text-1)] uppercase tracking-wider">
                          Field-by-Field Comparison
                        </h5>
                        <div className="overflow-x-auto rounded-xl border border-[var(--panel-border)] bg-[var(--panel-solid)] text-xs">
                          <table className="w-full text-left border-collapse">
                            <thead>
                              <tr className="bg-[var(--bg-2)] text-[11px] font-bold text-[var(--text-2)]">
                                <th className="p-2.5 border-b border-[var(--panel-border)] w-1/4">Field</th>
                                <th className="p-2.5 border-b border-[var(--panel-border)] w-3/8 text-sky-700 dark:text-sky-400">Your Local Data</th>
                                <th className="p-2.5 border-b border-[var(--panel-border)] w-3/8 text-emerald-700 dark:text-emerald-400">Server Cloud Data</th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-[var(--panel-border)]">
                              <tr>
                                <td className="p-2.5 font-semibold text-[var(--text-2)]">Report Title</td>
                              ...
                              <tr>
                                <td className="p-2.5 font-semibold text-[var(--text-2)]">Shutdown / Unit</td>
                              ...
                              <tr>
                                <td className="p-2.5 font-semibold text-[var(--text-2)]">5-Why Root Cause</td>
                              ...
                              <tr>
                                <td className="p-2.5 font-semibold text-[var(--text-2)]">Corrective Actions</td>
                                <td className="p-2.5">
                              ...
                              <tr>
                                <td className="p-2.5 font-semibold text-[var(--text-2)]">Spare Parts</td>
                                <td className="p-2.5">
        ...
        {/* Modal Footer */}
        <div className="p-4 sm:p-5 border-t border-[var(--panel-border)] bg-[var(--panel)] flex items-center justify-between">
          <div className="flex items-center space-x-2 text-xs text-[var(--text-2)]">
            <HelpCircle className="w-4 h-4 text-[var(--text-2)]" />
            <span>Resolving will immediately update your database and sync queues.</span>
          </div>
          <button
            onClick={onClose}
            className="px-5 py-2 bg-[var(--bg-2)] hover:bg-[var(--panel-border-strong)] text-[var(--text-0)] text-xs font-bold rounded-xl transition-colors cursor-pointer"
          >
            Close
          </button>
        </div>
      ...
      {inspectedReport && (
        ...
        >
          <div
            className="bg-[var(--panel-solid)] border border-[var(--panel-border)] rounded-3xl max-w-2xl w-full max-h-[85vh] flex flex-col shadow-2xl overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="p-5 border-b border-[var(--panel-border)] flex items-center justify-between bg-[var(--panel)]">
              <div className="flex items-center space-x-2">
              ...
              <button
                onClick={() => setInspectedReport(null)}
                className="p-1.5 text-[var(--text-2)] hover:text-[var(--text-1)] rounded-lg"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="p-6 overflow-y-auto space-y-4 text-xs">
              <div>
                <strong className="block text-[var(--text-2)] text-[10px] uppercase">Title</strong>
                <p className="font-bold text-[var(--text-0)] text-sm">{inspectedReport.data.title || 'Untitled'}</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <strong className="block text-[var(--text-2)] text-[10px] uppercase">Equipment</strong>
                  <p className="font-semibold text-[var(--text-0)]">{inspectedReport.data.equipmentName} ({inspectedReport.data.equipmentCode})</p>
                </div>
                <div>
                  <strong className="block text-[var(--text-2)] text-[10px] uppercase">Technician</strong>
                  <p className="font-semibold text-[var(--text-0)]">{inspectedReport.data.technicianName || 'N/A'}</p>
                </div>
              </div>
              <div>
                <strong className="block text-[var(--text-2)] text-[10px] uppercase">5-Why Analysis</strong>
                <ul className="list-disc list-inside space-y-1 text-[var(--text-1)] mt-1">
              ...
              {inspectedReport.data.notes && (
                <div>
                  <strong className="block text-[var(--text-2)] text-[10px] uppercase">Notes</strong>
                  <p className="text-[var(--text-1)] whitespace-pre-wrap">{inspectedReport.data.notes}</p>
                </div>
              )}
            </div>
            <div className="p-4 border-t border-[var(--panel-border)] flex justify-end">
              <button
                onClick={() => setInspectedReport(null)}
                className="px-4 py-2 bg-[var(--bg-2)] text-[var(--text-0)] rounded-xl text-xs font-bold"
              >
                Close Inspection
              </button>
            </div>
